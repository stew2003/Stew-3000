...
let rec desugar_expr (expr : expr) =
  ...
  | SPrefixInr (e, loc) ->
      ...
      (* e++ ==> e = e + 1 *)
      Assign (e_desugar, BinOp (Plus, e_desugar, NumLiteral (1, loc), loc), loc)
  | SPrefixDcr (e, loc) ->
      let e_desugar = desugar_expr e in
      (* e-- ==> e = e - 1 *)
      Assign (e_desugar, BinOp (Minus, e_desugar, NumLiteral (1, loc), loc), loc)
